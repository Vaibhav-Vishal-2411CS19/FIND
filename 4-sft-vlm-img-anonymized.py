...
def datatset_prep(tokenizer):
    ...
    dataset_split = dataset.train_test_split(test_size=0.3, seed=42, stratify_by_column="language_encoded") # 30% for testing
...
The question, an image and the options are given in {lang} language. Provide the answer option in the same language - {lang}
The question, an image and possible answers are as follows:"""
    ...
    test_dataset = [convert_to_conversation(sample) for sample in test_dataset]
...
def train_model(model, tokenizer, train_dataset, test_dataset, model_name):
    ...
    trainer = SFTTrainer(
        ...
        args = SFTConfig(
            per_device_train_batch_size = 8, #16 for all exp, other than Qwen2.5,32B,ep03 Try to keep this as high as possible!
            ...
            eval_strategy="steps",
            eval_steps=20, # 30 for exp, other than
...
for MODEL_NAME in model_list:
    ...
    try:
        ...
        SFT_MODEL_PATH = os.path.join(FT_LLM_DIR, MODEL_NAME.replace("/", "-") + "-sft-img-ep_01") # Change exp_id: img-ep01 as needed
