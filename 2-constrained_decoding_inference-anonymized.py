"""VQA Inference with vLLM - for MCQ type questions-
constained decoding - multimodal dataset"""
...
def _exp_init_ ():
    ...
    BASE_EXP_DATA_SAVE_FOLDER = f"./Experiments-infer_constrained_decoding-{datetime.now(tz=ZoneInfo('Asia/Kolkata')).strftime('%d_%b_%Y')}"
    ...
    logging.basicConfig(
        ...
        datefmt="%m-%d %H:%M:%S",
        filename=f"{BASE_EXP_DATA_SAVE_FOLDER}/inference_constrained_decoding{EXP_START_TIME}.log",
        force=True
    )
...
The question along with an image and possible options are given in {lang} language. Provide the answer option in the same language - {lang}
The question, image and the possible answers are as follows:"""}
        ],
    }]
...
def run_inference(llm: LLM, prompts: list, sampling_params: SamplingParams, lang: str) -> list[str]:
    """Run inference on all prompts."""
    logger.info("Running inference...")

    # constrained decoding for MCQ
    if lang in ['english', 'hindi', 'marathi']:
        sampling_params.max_tokens = 1
    else:
        sampling_params.max_tokens = 3
    POSSIBLE_CHOICES = [f"{LANG_CHAR_MAP.get(lang, 'ABCD')[0]}", f"{LANG_CHAR_MAP.get(lang, 'ABCD')[1]}", 
                        f"{LANG_CHAR_MAP.get(lang, 'ABCD')[2]}", f"{LANG_CHAR_MAP.get(lang, 'ABCD')[3]}"]
    logger.info(f"Possible choices for constrained decoding: {POSSIBLE_CHOICES}")
    logger.info(f"Number of maximum tokens generated: {sampling_params.max_tokens}")
    sampling_params.constrained_decoding_choices = [POSSIBLE_CHOICES]
...
def save_results(
    ...
    result.update({
        'model': model_id,
        'language': lang,
        'output_format': 'constrained_decoding',
...
def run_single_experiment(
    ...
    # Run inference
    predictions = run_inference(llm, prompts, sampling_params, lang)

    # Parse predictions
    processed_predictions = {"Answer": predictions} 
...
def model_path_sft(model_name: str) -> str:
    ...
    SFT_MODEL_PATH = os.path.join(FT_LLM_DIR, model_name.replace("/", "-") + "-sft-EXP_ID")  # Replace EXP_ID with actual experiment ID, eg. img-ep03
